        ...
        <div className="sidebar">
          ...
          <div className="specialty-buttons">
            {["General Physician", "Gynecologist", "Dermatologist", "Pediatrician", "Neurologist", "Gastroenterologist"].map(
              (specialty, index) => (
                <button key={index} onClick={() => fetchDoctorsBySpecialty(specialty)} className="specialty-button">
                  {specialty}
                </button>
              )
            )}
            ...
            doctors.map((doctor) => (
              <div key={doctor.user_id} className="doctor-card1">
                ...
                <div className="doctor-actions">
                  <button className="action-button" onClick={() => navigate("/patient/review")}>
                    Review
                  </button>
                  <button className="action-button" onClick={() => navigate("/patient/booking")}>
                    Make an Appointment
                  </button>
                </div>
              </div>
